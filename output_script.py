...
def handle_open(result):
    ...
    if not matches:
        return (False,f"No file named {filename} found in given path/folder.",(1, 1, 1),None,)
    # Filter exact matches (case-insensitive by name without extension)
    base_name = os.path.splitext(filename)[0].lower()
    exact_matches = [m for m in matches if os.path.splitext(os.path.basename(m))[0].lower() == base_name]
...
def handle_create(result):
    if not result["file"]:
        return False, "What would be the name of the file to create?", (0, 0, 1, 1)
    if not result["path"] and not result["folder"]:
        return False, "Where should the file be created?", (1, 1, 0, 0)
    filename = result["file"][0]
...
def handle_rename(result):
    ...
    exact_matches = [m for m in matches if os.path.splitext(os.path.basename(m))[0].lower() == base_name]
    if len(exact_matches) > 1:
        return (True,f"Multiple files found: {exact_matches}. Which one do you want to rename?",(1, 1, 1, 1),)
    elif len(exact_matches) == 1:
    ...
    if "." not in filename:
        if len(result["path"]) > 0:
            ...
            if len(searched_path) > 1:
                return (False,f"Multiple files found: {searched_path}. Which one do you want to move?",(1, 0, 1, 1),)
            elif len(searched_path) == 1:
                filename = searched_path[0]
            else:
                return (False,"File not found, please specify a correct filename or path.",(1, 0, 1, 1),)
        else:
    ...
    if "." not in filename:
        if len(result["path"]) > 0:
            ...
            for i in result["path"]:
                ...
                searched_path.extend(search if search else [])
            if len(searched_path) > 1:
                return (False,f"Multiple files found: {searched_path}. Which one do you want to copy?",(1, 0, 1, 1),)
            elif len(searched_path) == 1:
                filename = searched_path[0]
            else:
                return (False,"File not found, please specify a correct filename or path.",(1, 0, 1, 1),)
...
def handle_delete(result):
    ...
    exact_matches = [m for m in matches if os.path.splitext(os.path.basename(m))[0].lower() == base_name]
    if len(exact_matches) > 1:
        return (True,f"Multiple files found: {exact_matches}. Which one do you want to delete?",(1, 1, 1, 1),)
    elif len(exact_matches) == 1:
    ...
    if "." not in compressed_name:
        if len(result["path"]) > 0:
            ...
            for i in result["path"]:
                ...
                searched_path.extend(search if search else [])
            if len(searched_path) > 1:
                return (False,f"Multiple files found: {searched_path}. Which one do you want to compress?",(1, 0, 1, 1),)
            elif len(searched_path) == 1:
                compressed_name = searched_path[0]
            else:
                return (False,"File not found, please specify a correct filename or path.",(1, 0, 1, 1),)
        else:
            return (False,"What is the full compressed file name (with extension)?",(1, 0, 1, 1),)
...
def file_management_cmd_execution(command, cmd_id):
    ...
    if intent == "open":
        ...
        db.filepush(cmd_id,1,flag,result["file"] if db_input[0] == 1 else None,file_found,result["path"] if db_input[1] == 1 else None,result["folder"] if db_input[2] == 1 else None,message,)
        return message
    elif intent == "search":
