                ...
                for i in range(winreg.QueryInfoKey(key)[0]):
                    try:
                        ...
                        with winreg.OpenKey(key, subkey_name) as subkey:
                            app_display_name, _ = winreg.QueryValueEx(subkey, "DisplayName")
                            if app_display_name and app_name_lower in app_display_name.lower():
                                try:
                                    install_location, _ = winreg.QueryValueEx(subkey, "InstallLocation")
                                    if install_location:
                                        exe_path = Path(install_location) / (app_display_name + ".exe")
                                        if exe_path.exists():
                                            return str(exe_path)
...
def open_application(command):
    ...
    if app_path:
        if "UWP App ID" in app_path:
            ...
            subprocess.run(["cmd", "/c", "start", f"shell:AppsFolder\\{app_id}"], check=True)
        else:
...
def get_current_brightness():
    ...
    return brightness[0] if isinstance(brightness, list) else brightness  # Extract first element if list
...
def extract_intent(command):
    ...
    for token in doc:
        if any(token.lemma_ in ["open", "launch", "start"] for token in doc):
            return "open",None
        if any(token.lemma_ in ["force", "kill", "end", "immediately"] for token in doc):
            return "close","force"
        if any(token.lemma_ in ["close", "exit", "quit", "terminate", "stop"] for token in doc):
            return "close",None
        if token.lemma_ in ["increase", "decrease", "raise", "lower", "reduce"]:
            return "adjust", token.lemma_
        if token.lemma_ in ["set", "change"]:
            return "set", None
        if any(token.lemma_ in ["shutdown", "power", "turn"] for token in doc) and any(word.text in ["PC", "computer", "system"] for word in doc):
            return "shutdown",None
        if any(token.lemma_ in ["lock", "secure"] for token in doc):
            return "logout",None
        if any(token.lemma_ in ["restart", "reboot"] for token in doc):
            return "restart",None
        if any(token.lemma_ in ["sleep", "hibernate", "suspend", "standby"] for token in doc):
            return "sleep",None
    return None, None
